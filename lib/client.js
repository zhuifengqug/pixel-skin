...
`;
    ...
    function EffortPanel({ api, remote, sessionId }) {
      ...
      const load = React.useCallback(() => api.sessions.models({ sessionId }).then(response => {
        if (response.result.ok) setDirectory(response.result.value); else setError(response.result.error.message);
      }).catch(cause => setError(String(cause))), [api, sessionId]);
      ...
      React.useLayoutEffect(() => { setRawValue(usable ? selected * step : 0); setDragging(false); }, [directory, selected, step, usable]);
      const writeEffort = React.useCallback(value => {
        ...
        api.sessions.selectModel({ sessionId, provider: selection.current.provider, model: selection.current.model, reasoningEffort: effort.id }).then(response => {
          if (!response.result.ok) setError(response.result.error.message); else setError(null);
        }).catch(cause => setError(String(cause)));
      }, [api, efforts, selection, sessionId, step, usable]);
      const onInput = event => {
      ...
      if (directory === null) return React.createElement("div", { className: "pixel-effort-status" }, copy.loading);
    ...
    function installEffortPicker(ctx) {
      const connection = ctx.get("connection"); const sessions = ctx.get("sessions");
      if (!connection || !sessions || typeof document === "undefined") return () => {};
      const host = document.createElement("div"); host.className = "pixel-effort-host"; document.body.appendChild(host);
      const api = connection.api; let root = null; let hiddenMenu = null;
      ...
      const show = (sessionId, anchor, menu) => {
        ...
        root.render(React.createElement(EffortPanel, { api, remote: ctx.get("remote") || ctx.remote, sessionId }));
      };
      const addAutoEntry = sessionId => api.sessions.models({ sessionId }).then(response => {
        if (!response.result.ok) return; const selection = currentModel(response.result.value); if (!selection || (selection.model.reasoning?.efforts || []).length) return;
    ...
    // ---- 插件体 ----
    const inject = ["slots", "theme", "connection", "sessions", "remote"];
